...
void print_results(long cut_count, clock_t time0, clock_t time1, clock_t time2, double total_e);
...
int main(int argc, char *argv[])
{
    ...
    /* Step 3 - Allocate the arrays to store the coordinate and charge
       data */
    coords=alloc_2D_double(natom,3);
    if ( coords==NULL )
    {
    ...
    for (i=2; i<=natom; ++i)
    {
        ...
        for (j=1; j<i; ++j)
        {
            double* temp = coords[j-1];
            double j_0 = temp[0];
            double j_1 = temp[1];
            double j_2 = temp[2];

            /* X^2 + Y^2 + Z^2 */
            vec2 = (i_0-j_0)*(i_0-j_0)
                   + (i_1-j_1)*(i_1-j_1)
                   + (i_2-j_2)*(i_2-j_2);
    ...
    print_results(cut_count, time0, time1, time2, total_e);
